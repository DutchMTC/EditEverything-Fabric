...
public class GuiAdventureBlockModifier extends GuiListModifier<ItemStack> {
    ...
    public enum Kind {
        CAN_BREAK("gui.act.modifier.meta.canBreak", DataComponents.CAN_BREAK, "CanDestroy"),
        CAN_PLACE("gui.act.modifier.meta.canPlace", DataComponents.CAN_PLACE_ON, "CanPlaceOn");
    ...
    private static final Component NOTE = Component.translatable("gui.act.modifier.meta.adventure.note");
    ...
    private static class AddBlockListElement extends ListElement {
        public AddBlockListElement(GuiAdventureBlockModifier parent, Component label) {
            super(LIST_WIDTH, 24);
            buttonList.add(new ACTButton(0, 2, LIST_WIDTH, 20, label, b -> parent.openBlockSelector()));
        }
    }
    ...
    private GuiAdventureBlockModifier(Screen parent, ItemStack stack, Kind kind, Mode mode, Set<String> originalAllowedIds,
        ...
        addListElement(new AddBlockListElement(this, Component.translatable("gui.act.modifier.meta.adventure.add")));
    ...
    public void init() {
        ...
        addRenderableWidget(new ACTButton(width / 2 - 100, height - 42, 99, 20, getModeButtonText(), b -> toggleMode()));
    }
    ...
    private String getModeLine() {
        String modeLabel = I18n.get(mode == Mode.ONLY
                ? "gui.act.modifier.meta.adventure.mode.only"
                : "gui.act.modifier.meta.adventure.mode.except");
        int total = allBlockIds.size();
        if (mode == Mode.ONLY) {
            int allowedCount = getAllowedIds().size();
            return I18n.get("gui.act.modifier.meta.adventure.mode") + ": " + modeLabel
                    + " (" + allowedCount + "/" + total + ")";
        }
    ...
    private Component getModeButtonText() {
        return Component.literal(I18n.get("gui.act.modifier.meta.adventure.mode") + ": ")
                .append(Component.translatable(mode == Mode.ONLY
                        ? "gui.act.modifier.meta.adventure.modebtn.only"
                        : "gui.act.modifier.meta.adventure.modebtn.except"));
    }
    ...
    private void openBlockSelector() {
        ...
        getMinecraft().setScreen(new GuiTypeListSelector(this,
                Component.translatable("gui.act.modifier.meta.adventure.selectBlock"),
                chosen -> {
