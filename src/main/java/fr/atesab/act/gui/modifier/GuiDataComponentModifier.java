...
public class GuiDataComponentModifier extends GuiListModifier<ItemStack> {
    ...
    private static class ComponentListElement extends ListElement {
        ...
        public ComponentListElement(GuiDataComponentModifier parent, DataComponentType<?> type, String id) {
            ...
            int buttonY = 7;
            buttonList.add(new ACTButton(LIST_WIDTH - 166, buttonY, 60, 20,
                    Component.translatable("gui.act.modifier.meta.dataComponents.clone"),
                    b -> parent.openCloneSelector(type)));
            buttonList.add(new ACTButton(LIST_WIDTH - 104, buttonY, 60, 20,
                    Component.translatable("gui.act.modifier.meta.dataComponents.edit"),
                    b -> parent.openEditor(type)));
            buttonList.add(new ACTButton(LIST_WIDTH - 42, buttonY, 40, 20,
    ...
    private static class AddComponentListElement extends ListElement {
        public AddComponentListElement(GuiDataComponentModifier parent) {
            super(LIST_WIDTH, 24);
            buttonList.add(new ACTButton(0, 2, LIST_WIDTH, 20,
                    Component.translatable("gui.act.modifier.meta.dataComponents.add"),
    ...
    private GuiDataComponentModifier(Screen parent, ItemStack stack, List<ListElement> elements) {
        super(parent, Component.translatable("gui.act.modifier.meta.dataComponents"), elements, s -> {
        }, new Tuple[0]);
        this.stack = stack;
    ...
    private String describeComponentValue(DataComponentType<?> type) {
        ...
        try {
            ...
            return "<" + I18n.get("gui.act.modifier.meta.dataComponents.encodeError") + ">";
        } catch (Throwable t) {
            return "<" + I18n.get("gui.act.modifier.meta.dataComponents.encodeError") + ">";
        }
    }
    ...
    private void openCloneSelector(DataComponentType<?> type) {
        getMinecraft().setScreen(new fr.atesab.act.gui.selector.GuiTypeListSelector(this,
                Component.translatable("gui.act.modifier.meta.dataComponents.cloneFrom"),
                example -> {
                    Object exampleValue = ItemUtils.getComponent(example, (DataComponentType<Object>) type);
                    if (exampleValue == null) {
                        ChatUtils.error(I18n.get("gui.act.modifier.meta.dataComponents.cloneMissing"));
                        return this;
                    }
                    Tag encoded;
                    try {
                        ...
                        ChatUtils.error(I18n.get("gui.act.modifier.meta.dataComponents.encodeError") + ": " + t.getMessage());
                        return this;
                    }
    ...
    private Screen buildEditorScreen(DataComponentType<?> type, Tag encodedOverride, boolean forceGeneric) {
        ...
        if (encodedOverride != null) {
            ...
            try {
                ...
                if (encodedTag == null && encoded.error().isPresent()) {
                    ChatUtils.error(I18n.get("gui.act.modifier.meta.dataComponents.encodeError") + ": "
        ...
        return new GuiNBTModifier(Component.literal(idStr), back, editedWrapper -> {
            ...
            if (valueTag == null) {
                ChatUtils.error(I18n.get("gui.act.modifier.meta.dataComponents.missingValue"));
                return;
            }
            ...
            if (out.isEmpty()) {
                String msg = decoded.error().map(e -> e.message()).orElse(I18n.get("gui.act.modifier.meta.dataComponents.decodeError"));
                ChatUtils.error(I18n.get("gui.act.modifier.meta.dataComponents.decodeError") + ": " + msg);
                return;
            }
